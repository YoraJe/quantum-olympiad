...
export function LoginPage({ onLoginSuccess }: LoginPageProps) {
  ...
  const handleLogin = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      // CRITICAL: Role validation for admin mode
  ...
  const fillDemo = useCallback((type: 'student' | 'admin') => {
    if (type === 'admin') {
      setEmail('admin@quantum.id');
      setAuthView('admin-login');
    } else {
      setEmail('student@quantum.id');
      setAuthView('student-login');
    }
    setPassword('demo');
    setError('');
    ...
    >
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          transition={{ duration: 0.4, ease: [0.4, 0, 0.2, 1], layout: { duration: 0.35 } }}
          className={cn(
        ...
        >
          ...
          {isAdmin && (
            <div className="absolute inset-0 pointer-events-none z-0 scanline-overlay" />
          )}
...
function RegisterHeader({ onBack }: { onBack: () => void }) {
  return (
    <>
      <button
        onClick={onBack}
        className="flex items-center gap-1.5 text-white/30 hover:text-white/60 transition-colors mb-3"
      >
      ...
      <h2 className="text-2xl font-bold text-white">
        Join the <span className="bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">Academy</span>
      </h2>
      <p className="text-white/40 text-sm mt-1">
...
function AdminHeader() {
  return (
    <>
      ...
      <h2 className="text-xl font-bold font-mono text-green-400 tracking-wider">
        SYSTEM OVERRIDE
      </h2>
      <motion.p
        className="text-green-400/50 text-xs font-mono mt-1 tracking-wider"
